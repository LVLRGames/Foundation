...
func register_record(record: FoundationSpatialRecord) -> bool:
	...
	for chunk in chunks:
		unique_regions[coordinate_system.chunk_to_region(chunk)] = true
	var regions: Array[Vector2i] = []
	for region: Vector2i in unique_regions:
	...
	for region in record.owning_regions:
		if not _region_buckets.has(region):
			_region_buckets[region] = {}
		var bucket: Dictionary = _region_buckets[region]
		bucket[record.stable_id] = true
	return true
...
func unregister_record(stable_id: StringName) -> bool:
	...
	for region in record.owning_regions:
		var bucket: Dictionary = _region_buckets.get(region, {})
		bucket.erase(stable_id)
		if bucket.is_empty():
			_region_buckets.erase(region)
